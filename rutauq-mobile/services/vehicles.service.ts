...
export const vehiclesService = {
  ...
  async create(data: CreateVehicleRequest): Promise<VehicleSummary> {
    const res = await apiClient.post<ApiResponse<VehicleSummary>>('/vehicles', data);
    return res.data.data;
  },

  async update(id: string, data: Partial<CreateVehicleRequest>): Promise<VehicleSummary> {
    const res = await apiClient.put<ApiResponse<VehicleSummary>>(`/vehicles/${id}`, data);
    return res.data.data;
  },
